...
DefaultNorm2d = partial(LayerNorm2d, eps=1e-6)
DefaultAct = nn.GELU
...
def vit_base(image_size: int = 1024):
    """vit base
    """
    vit = ImageEncoderViT(
...
def vit_large(image_size: int = 1024):
    """vit large
    """
    vit = ImageEncoderViT(
        image_size=image_size,
...
def vit_huge(image_size: int = 1024):
    """vit huge
    """
    vit = ImageEncoderViT(
        image_size=image_size,
...
class SamVitAdapter(nn.Module):
    ...
    def __init__(
            self,
            input_img_size: Union[int, Tuple[int, int]] = 640,
        ...
        if isinstance(input_img_size, int):
            input_img_size = (input_img_size, input_img_size)
        self.vit = globals()[backbone_name](input_img_size)
        if pretrained:
            adapt_model(self.vit, img_size=input_img_size, backbone_name=backbone_name)

        state = self.training
        self.train(False)
        for output in self(torch.rand((1, 3, input_img_size[0], input_img_size[1]), dtype=torch.float32)):
            assert isinstance(output, torch.Tensor)
            assert len(output.shape) == 4
            _, c, h, w = output.shape
            self.ch_out_list.append(int(c))
            self.stride_list.append(int(input_img_size[0] / h))
        self.train(state)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.vit.pos_embed is not None:
            x = x + adapt_pos_embed(self.vit.pos_embed, (h, w))

        for i, blk in enumerate(self.vit.blocks):
            x = blk(x)
            if i in self.vit.global_attn_indexes:
...
def test():
    state = torch.load('/mnt/cephfs/data/pretrained_weight/sam/sam_vit_b_01ec64.pth')
    check_point = {}
    for k, v in state.items():
        if k.startswith('image_encoder'):
            check_point[k.replace('image_encoder.', '')] = v
