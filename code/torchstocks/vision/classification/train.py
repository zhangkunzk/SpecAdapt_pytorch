...
from torchstocks.vision.classification.dataset import TrainDataset, TestDataset
from torchstocks.vision.classification.dataset import  TestTransform, InferenceTransform
from torchstocks.vision.classification.trainer import ClassificationTrainer
...
class ModelEntry(AbstractModelEntry):
    ...
    def __init__(self, args):
        ...
        if args.pretrained_params_file:
            ...
            pretrained_params = torch.load(
                args.pretrained_params_file, map_location='cpu')
    ...
    def train(self):
        ...
        if args.train_file or args.train_data_path:
            ...
            train_dataset_config.path = os.path.join(
                args.train_data_path, args.train_file)
        ...
        if args.test_file or args.test_data_path:
            ...
            test_dataset_config.path = os.path.join(
                args.test_data_path, args.test_file)
    ...
    def do_inference(self, infer_model, image):
        ...
        input_tensor = torch.from_numpy(
            resized_image).unsqueeze(0).to(self.device)
        results = infer_model(input_tensor)[0]
        score, idx = results.max(dim=0)
        return {
            'class_id': int(idx.detach().cpu().numpy()),
            'confidence': float(score.detach().cpu().numpy())
        }
    ...
    def inference(self, infer_model_file, input_image):
        ...
        Returns:
            results: ndarray or ndarray sequence, for per image's class
        """
        self.inference_transform = InferenceTransform(
...
def main():
    ...
    parser.add_argument('--eval_every_epoch', type=int,
                        default=5, help='eval every ? epoch')
    ...
    if train_dataset:
        trainer.train()
        progress = trainer.get_status('progress')
        print('progress: ', progress)
        if 'metrics' in trainer.status:
